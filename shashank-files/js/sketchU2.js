...
function setMolecule(ID) {
    ...
    if (ID == 1) {
        ...
        setup();
        img = loadImage('Calcium-Chloride.png');
...
stack[stackCounter] = Composites.softBody(650, 20, 10, 5, 0, 0, true, 15, particleOptions);
...
///////////////caCl
    for (var i = 0; i < bodies.length; i++) {
        ...
        if(bodyType[i]==undefined)
            bodyType[i] = 3;
    	bodynumber[i].collideFlag=-1;
    	bodies[i].collideFlag=-1;
    }
        ...
        for (var i = 0; i < ClParticles.length; i++) {
         ...
         /*for (var i = 0; i < NaParticles.length; i++) {
            for (var j = 0; j < NaParticles.length; j++) {
                ...
                var pvector = createVector((NaParticles[i].position.x),(NaParticles[i].position.y));
                var p2vector = createVector((ClParticles[j].position.x),(ClParticles[j].position.y));
                var distforce = dist(NaParticles[i].position.x, NaParticles[i].position.y, ClParticles[j].position.x, ClParticles[j].position.y);
                var diffvector = p5.Vector.sub(pvector, p2vector);
                diffvector.normalize();
                diffvector.mult(mag*2);
                if (distforce > 40) {
                    Matter.Body.applyForce(NaParticles[j], pvector, diffvector);
                }
                }
            }
        } */
...
function addMoleculesBtnNacl(){
    addMoleculesNacl(document.getElementById("moleculevalNacl").innerHTML,molID);
}

function addMoleculesNacl(number,ID)
{
 ...
 totalMolecules1 += parseInt(number);
   ...
   if(molID == 1)
   {
   for (var i= 0,x=400; i< number;i++,x+=40)
   {
   if(i%2 == 0)
   {
   y1 = 200;
   ...
   {
   NaParticles.push(new particleNa(x,y));
   ClParticles.push(new particleCl(x+40,y));
   ...
   if (NaParticles.length < 5)
    ...
    if (molID ==6)
    {
    var constraint = Constraint.create(options1);
    }
